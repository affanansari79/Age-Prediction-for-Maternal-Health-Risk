...
with tab2:
    ...
    st.markdown("**Formula:** `Age ~ SystolicBP + BS + BS2 + BodyTemp`  \n*(Age is √-transformed; metrics are back-transformed)*")
    coef_df = pd.DataFrame({
        "Coefficient": lm.params, "Std Error": lm.bse,
        "t-value": lm.tvalues, "p-value": lm.pvalues,
    }).round(4)
    ...
    ax2.set_title("OLS Coefficients (excl. Intercept)"); ax2.set_ylabel("Value")
...
with tab3:
    ...
    with col_a:
        ...
        ax3.set_title("Histogram of Residuals"); ax3.set_xlabel("Error"); ax3.set_ylabel("Frequency"); ax3.legend()
        st.pyplot(fig3); plt.close(fig3)
    with col_b:
        fig4, ax4 = plt.subplots()
        ax4.scatter(real_y_pred, error, alpha=0.4, color="#1abc9c")
        ax4.axhline(0, color="red", linestyle="--")
        ax4.set_title("Residuals vs Predicted Age"); ax4.set_xlabel("Predicted Age"); ax4.set_ylabel("Residual")
        st.pyplot(fig4); plt.close(fig4)
    st.subheader("Actual vs Predicted Age")
    fig5, ax5 = plt.subplots()
    ax5.scatter(real_y_test, real_y_pred, alpha=0.4, color="#e67e22")
    mn = min(real_y_test.min(), real_y_pred.min()); mx = max(real_y_test.max(), real_y_pred.max())
    ax5.plot([mn, mx], [mn, mx], "r--", label="Perfect fit")
    ax5.set_xlabel("Actual Age"); ax5.set_ylabel("Predicted Age"); ax5.set_title("Actual vs Predicted (test set)"); ax5.legend()
...
with tab4:
    ...
    # ✅ Each widget has a unique key= to prevent DuplicateElementId error
    ...
    if st.button("Predict Age", type="primary", key="predict_btn"):
        ...
        st.caption("Model R² ≈ 0.41 — use as a statistical estimate, not a clinical decision.")    df["Age"] = np.sqrt(df["Age"])
    return df, ss, cont_cols
...
with tab4:
    ...
    def num_input(label, col):
        return st.number_input(label,
            min_value=float(raw_stats.loc["min", col]),
            max_value=float(raw_stats.loc["max", col]),
            value=float(raw_stats.loc["50%", col]),
            step=1.0)
    ...
    if st.button("Predict Age", type="primary"):
        # Fill dummy row with column means, override our 2 inputs
        col_means = raw_df[cont_cols].mean()
        dummy_dict = {col: [float(col_means[col])] for col in cont_cols}
        dummy_dict["SystolicBP"] = [float(systolic)]
        dummy_dict["BS"]         = [float(bs)]
